...
export const create = asyncHandler(async (req, res) => {
  ...
  if (req.body.batch && String(req.body.batch).trim()) {
    const batchName = String(req.body.batch).trim().toLowerCase();
    const existingBatch = await Batch.findOne({ name: batchName, active: true }).select("name");
    if (!existingBatch) {
      return res.status(400).json({ message: `Batch "${req.body.batch}" does not exist. Please create the batch first.` });
    }
    req.body.batch = existingBatch.name;
  }
