...
export default function Employees() {
  ...
  useHeaderActions(
    <button
      onClick={() => { setForm({ ...emptyForm, joined: todayStr() }); setModalOpen(true); }}
  ...
  const startEdit = (t) => {
    ...
    window.dispatchEvent(new CustomEvent("AI_EDIT_CONTEXT", { detail: { entity: "employee", targetId: t.id, currentForm } }));
    setEditTarget(t);
...
<body>
  ...
  useEffect(() => {
    const handleOpen = (e) => {
      ...
      setForm((current) => ({
        ...
        // Every NEW employee form starts with today's browser-local date.
    ...
    window.addEventListener("AI_OPEN_EMPLOYEE_MODAL", handleOpen);
    window.addEventListener("AI_UPDATE_EMPLOYEE_FORM", handleOpen);
    window.addEventListener("AI_SAVE_EMPLOYEE", handleSave);
    window.addEventListener("AI_CANCEL_VOICE_FORMS", handleCancel);
    return () => {
      window.removeEventListener("AI_OPEN_EMPLOYEE_MODAL", handleOpen);
      window.removeEventListener("AI_UPDATE_EMPLOYEE_FORM", handleOpen);
      window.removeEventListener("AI_SAVE_EMPLOYEE", handleSave);
      window.removeEventListener("AI_CANCEL_VOICE_FORMS", handleCancel);
    };
  }, []);

  useEffect(() => {
    const update = (e) => { const d = e.detail?.formData || {}; setEditForm((c) => ({ ...c, ...(d.name !== undefined ? { name: d.name } : {}), ...(d.specialization !== undefined ? { specialization: d.specialization } : {}), ...(d.salary !== undefined ? { salary: d.salary } : {}), ...(d.joiningDate !== undefined ? { joined: d.joiningDate } : {}) })); };
  ...
  useEffect(() => {
    ...
    const execute = async (operation, item) => {
      ...
      if (operation === 'delete') { try { await employeesApi.remove(item.id); setEmployees((prev) => prev.filter((t) => t.id !== item.id)); result(`${item.name} was deleted successfully.`); } catch (err) { result(`I could not delete ${item.name}: ${err.message}`); } return; }
      if (operation === 'activate' || operation === 'deactivate') { const next = operation === 'activate'; if (Boolean(item.active) === next) { result(`${item.name} is already ${next ? 'active' : 'inactive'}.`); return; } try { await employeesApi.update(item.id, { active: next }); loadEmployees(); result(`${item.name} was ${next ? 'activated' : 'deactivated'} successfully.`); } catch (err) { result(`I could not change ${item.name}'s status: ${err.message}`); } }
    };
    const handler = (e) => { const { operation, query: rawQuery } = e.detail || {}; const q = norm(rawQuery); if (!q) return; let matches = employees.filter((t) => norm(t.name) === q || norm(t.specialization) === q); if (!matches.length) matches = employees.filter((t) => norm(t.name).includes(q) || norm(t.specialization).includes(q)); if (!matches.length) { result(`I could not find an employee matching ${rawQuery}.`); return; } if (matches.length > 1) { ambiguous({ entity: 'employee', operation, manual: true, message: `I can see multiple employees matching ${rawQuery}. For safety, please manually visit the employee record and choose View, Edit, Activate/Deactivate, or Delete.` }); return; } execute(operation, matches[0]); };
      if (loading) { setTimeout(() => handler(e), 350); return; }
    const executeSelected = (e) => { const { entity, operation, candidate } = e.detail || {}; if (entity === 'employee' && candidate?.id) { const item = employees.find((t) => t.id === candidate.id); if (item) execute(operation, item); } };
