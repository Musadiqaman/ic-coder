...
export default function Expenses() {
  ...
  useHeaderActions(
    <button
      onClick={() => { setForm({ ...emptyForm, date: todayStr() }); setModalOpen(true); }}
  ...
  const startEdit = (e) => {
    const currentForm = { title: e.title, amount: e.amount, description: e.description, date: e.date };
    window.dispatchEvent(new CustomEvent("AI_EDIT_CONTEXT", { detail: { entity: "expense", targetId: e.id, currentForm } }));
    setEditTarget(e);
  ...
  useEffect(() => {
    const handleOpen = (e) => {
      ...
      setForm((current) => ({
        ...
        // Every NEW expense form starts with today's browser-local date.
    ...
    window.addEventListener("AI_OPEN_EXPENSE_MODAL", handleOpen);
    window.addEventListener("AI_UPDATE_EXPENSE_FORM", handleOpen);
    window.addEventListener("AI_SAVE_EXPENSE", handleSave);
    window.addEventListener("AI_CANCEL_VOICE_FORMS", handleCancel);
    return () => {
      window.removeEventListener("AI_OPEN_EXPENSE_MODAL", handleOpen);
      window.removeEventListener("AI_UPDATE_EXPENSE_FORM", handleOpen);
      window.removeEventListener("AI_SAVE_EXPENSE", handleSave);
      window.removeEventListener("AI_CANCEL_VOICE_FORMS", handleCancel);
    };
  }, []);

  useEffect(() => {
    const update = (e) => { const d = e.detail?.formData || {}; setEditForm((c) => ({ ...c, ...(d.title !== undefined ? { title: d.title } : {}), ...(d.amount !== undefined ? { amount: d.amount } : {}), ...(d.description !== undefined ? { description: d.description } : {}), ...(d.date !== undefined ? { date: d.date } : {}) })); };
  ...
  useEffect(() => {
    ...
    const execute = async (operation, item) => {
      if (operation === 'view') { result(`Expense: ${item.title}. Amount ${pkr(item.amount)}. ${item.description || ''} Date ${item.date}.`); return; }
      if (operation === 'edit') { startEdit(item); result(`Opening expense ${item.title} for editing.`); return; }
      if (operation === 'delete') { try { await expensesApi.remove(item.id); setExpenses((prev) => prev.filter((e) => e.id !== item.id)); result(`Expense ${item.title} was deleted successfully.`); } catch (err) { result(`I could not delete expense ${item.title}: ${err.message}`); } }
    };
    const handler = (e) => { const { operation, query: rawQuery } = e.detail || {}; const q = norm(rawQuery); if (!q) return; let matches = expenses.filter((x) => norm(x.title) === q || norm(x.description) === q); if (!matches.length) matches = expenses.filter((x) => norm(x.title).includes(q) || norm(x.description).includes(q)); if (!matches.length) { result(`I could not find an expense matching ${rawQuery}.`); return; } if (matches.length > 1) { ambiguous({ entity: 'expense', operation, message: `I found multiple expenses matching ${rawQuery}. Please say the index number you want.`, candidates: matches.map((x) => ({ id: x.id, label: x.title, details: `${pkr(x.amount)} · ${x.date}` })) }); return; } execute(operation, matches[0]); };
      if (loading) { setTimeout(() => handler(e), 350); return; }
    const executeSelected = (e) => { const { entity, operation, candidate } = e.detail || {}; if (entity === 'expense' && candidate?.id) { const item = expenses.find((x) => x.id === candidate.id); if (item) execute(operation, item); } };
