...
export default function Projects() {
  ...
  const startEdit = (p) => {
    const currentForm = { name: p.name, ownerName: p.ownerName, totalCost: p.totalCost, ownerPhone: p.ownerPhone };
    window.dispatchEvent(new CustomEvent("AI_EDIT_CONTEXT", { detail: { entity: "project", targetId: p.id, currentForm } }));
    setEditTarget(p);
...
<body>
  ...
  useEffect(() => {
    const handleOpen = (e) => {
      ...
      setForm((current) => ({
        ...
        totalCost: data.totalCost ?? current.totalCost ?? "",
        ownerPhone: data.ownerPhone ?? current.ownerPhone ?? "",
      }));
      setModalOpen(true);
    };
    const handleSave = () => voiceSubmitRef.current?.();

    window.addEventListener("AI_OPEN_PROJECT_MODAL", handleOpen);
    window.addEventListener("AI_UPDATE_PROJECT_FORM", handleOpen);
    window.addEventListener("AI_SAVE_PROJECT", handleSave);
    return () => {
      window.removeEventListener("AI_OPEN_PROJECT_MODAL", handleOpen);
      window.removeEventListener("AI_UPDATE_PROJECT_FORM", handleOpen);
      window.removeEventListener("AI_SAVE_PROJECT", handleSave);
    };
  }, []);
  ...
  useEffect(() => {
    const update = (e) => { const d = e.detail?.formData || {}; setEditForm((c) => ({ ...c, ...(d.name !== undefined ? { name: d.name } : {}), ...(d.ownerName !== undefined ? { ownerName: d.ownerName } : {}), ...(d.totalCost !== undefined ? { totalCost: d.totalCost } : {}), ...(d.ownerPhone !== undefined ? { ownerPhone: d.ownerPhone } : {}) })); };
  ...
  useEffect(() => {
    ...
    const execute = async (operation, item) => {
      if (operation === 'view') { setViewTargetId(item.id); setDetailTab('challans'); result(`Opening project ${item.name}.`); return; }
      if (operation === 'edit') { startEdit(item); result(`Opening project ${item.name} for editing.`); return; }
      if (operation === 'delete') { try { await projectsApi.remove(item.id); setProjects((prev) => prev.filter((p) => p.id !== item.id)); result(`Project ${item.name} was deleted successfully.`); } catch (err) { result(`I could not delete project ${item.name}: ${err.message}`); } }
    };
    const handler = (e) => {
      if (loading) { setTimeout(() => handler(e), 350); return; }
      const { operation, query: rawQuery, qualifier } = e.detail || {}; const q = norm(rawQuery); if (!q) return;
      let matches = projects.filter((p) => norm(p.name) === q);
      if (!matches.length) matches = projects.filter((p) => norm(p.name).includes(q));
      if (!matches.length) { result(`I could not find a project matching ${rawQuery}.`); return; }
      if (matches.length > 1 && qualifier?.type === 'owner') { const oq = norm(qualifier.value); const ownerMatches = matches.filter((p) => norm(p.ownerName) === oq || norm(p.ownerName).includes(oq)); if (ownerMatches.length === 1) { execute(operation, ownerMatches[0]); return; } if (ownerMatches.length === 0) { result(`I found projects named ${rawQuery}, but none has owner ${qualifier.value}. Please provide the correct project owner name.`); return; } matches = ownerMatches; }
      if (matches.length > 1) { ambiguous({ entity: 'project', operation, manual: true, awaitQualifier: true, originalQuery: rawQuery, message: `I can see multiple projects named ${rawQuery}. Please provide the project owner name so I can identify the correct project.` }); return; }
      execute(operation, matches[0]);
    };
    const executeSelected = (e) => { const { entity, operation, candidate } = e.detail || {}; if (entity === 'project' && candidate?.id) { const item = projects.find((p) => p.id === candidate.id); if (item) execute(operation, item); } };
