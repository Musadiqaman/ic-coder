...
export default function Loans() {
  ...
  const startEdit = (l) => {
    const currentForm = { from: l.from, kind: l.kind, amount: l.amount, contact: l.contact };
    window.dispatchEvent(new CustomEvent("AI_EDIT_CONTEXT", { detail: { entity: "loan", targetId: l.id, currentForm } }));
    setEditTarget(l);
  ...
  useEffect(() => {
    const handleOpen = (e) => {
      ...
      setForm((current) => ({
        ...
        kind: data.kind ?? current.kind ?? "person",
        amount: data.amount ?? current.amount ?? "",
        contact: data.contact ?? current.contact ?? "",
      }));
      setModalOpen(true);
    };
    const handleSave = () => voiceSubmitRef.current?.({ preventDefault() {} });

    window.addEventListener("AI_OPEN_LOAN_MODAL", handleOpen);
    window.addEventListener("AI_UPDATE_LOAN_FORM", handleOpen);
    window.addEventListener("AI_SAVE_LOAN", handleSave);
    return () => {
      window.removeEventListener("AI_OPEN_LOAN_MODAL", handleOpen);
      window.removeEventListener("AI_UPDATE_LOAN_FORM", handleOpen);
      window.removeEventListener("AI_SAVE_LOAN", handleSave);
    };
  }, []);
  ...
  useEffect(() => {
    const update = (e) => { const d = e.detail?.formData || {}; setEditForm((c) => ({ ...c, ...(d.from !== undefined ? { from: d.from } : {}), ...(d.kind !== undefined ? { kind: d.kind } : {}), ...(d.amount !== undefined ? { amount: d.amount } : {}), ...(d.contact !== undefined ? { contact: d.contact } : {}) })); };
  ...
  useEffect(() => {
    ...
    const execute = async (operation, item) => {
      if (operation === 'view') { setViewTargetId(item.id); result(`Opening loan for ${item.from}.`); return; }
      if (operation === 'edit') { startEdit(item); result(`Opening the loan for ${item.from} for editing.`); return; }
      if (operation === 'delete') { try { await loansApi.remove(item.id); setLoans((prev) => prev.filter((l) => l.id !== item.id)); result(`Loan for ${item.from} was deleted successfully.`); } catch (err) { result(`I could not delete the loan for ${item.from}: ${err.message}`); } }
    };
    const handler = (e) => {
      ...
      let matches = loans.filter((l) => norm(l.from) === q || norm(l.contact) === q || norm(l.kind) === q);
      if (!matches.length) matches = loans.filter((l) => norm(l.from).includes(q) || norm(l.contact).includes(q));
      if (!matches.length) { result(`I could not find a loan matching ${rawQuery}.`); return; }
      if (matches.length > 1 && qualifier?.type === 'contact') { const cq = digits(qualifier.value); const cm = matches.filter((l) => digits(l.contact) === cq); if (cm.length === 1) { execute(operation, cm[0]); return; } if (!cm.length) { result(`I found multiple loans for ${rawQuery}, but no matching contact number. Please provide the correct contact number.`); return; } matches = cm; }
      if (matches.length > 1) { ambiguous({ entity: 'loan', operation, manual: true, awaitQualifier: true, originalQuery: rawQuery, message: `I found multiple loans for ${rawQuery}. Please provide the person's or company's contact number so I can identify the correct loan.` }); return; }
      execute(operation, matches[0]);
    };
    const executeSelected = (e) => { const { entity, operation, candidate } = e.detail || {}; if (entity === 'loan' && candidate?.id) { const item = loans.find((x) => x.id === candidate.id); if (item) execute(operation, item); } };
