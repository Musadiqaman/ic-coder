...
export default function Teachers() {
  ...
  useHeaderActions(
    <>
      ...
      <button onClick={() => { setForm({ ...emptyForm, joined: todayStr() }); setModalOpen(true); }} className="flex items-center gap-1.5 sm:gap-2 rounded-xl px-2.5 py-2 sm:px-4 sm:py-2.5 text-xs sm:text-sm font-semibold shrink-0 transition-all active:scale-95" style={{ background: C.gold, color: "#fff" }}>
  ...
  const startEdit = (t) => {
    const currentForm = { name: t.name, specialization: t.specialization, salary: t.salary, joined: t.joined };
    window.dispatchEvent(new CustomEvent("AI_EDIT_CONTEXT", { detail: { entity: "teacher", targetId: t.id, currentForm } }));
    setEditTarget(t);
...
<body>
  ...
  voiceEditSubmitRef.current = typeof submitEdit === "function" ? () => submitEdit({ preventDefault() {} }) : null;
  ...
  useEffect(() => {
    const resolveVoiceBatchIds = (data, currentIds = []) => {
      const rawNames = [];
      if (data?.batch) rawNames.push(String(data.batch));
      if (Array.isArray(data?.batchNames)) rawNames.push(...data.batchNames.map(String));
      const names = rawNames.map((v) => v.trim().toLowerCase()).filter(Boolean);
      if (!names.length) return currentIds || [];

      const matchedIds = batches
        .filter((b) => names.includes(String(b.name).trim().toLowerCase()))
        .map((b) => b._id);
      const missing = names.filter((name) => !batches.some((b) => String(b.name).trim().toLowerCase() === name));
      if (missing.length) {
        setTimeout(() => window.dispatchEvent(new CustomEvent("AI_VOICE_NOTICE", {
          detail: { message: `I could not find the batch${missing.length > 1 ? "es" : ""} ${missing.map((x) => `"${x}"`).join(", ")}. Please create it first.` }
        })), 0);
      }
      return [...new Set([...(currentIds || []), ...matchedIds])];
    };
    ...
    const handleOpen = (e) => {
      ...
      setForm((current) => ({
        ...
        accountEmail: data.accountEmail ?? current.accountEmail ?? "",
        accountPassword: data.accountPassword ?? current.accountPassword ?? "",
        batchIds: resolveVoiceBatchIds(data, e.type === "AI_OPEN_TEACHER_MODAL" ? [] : (current.batchIds || [])),
        // Every NEW teacher form starts with today's browser-local date.
        // Spoken/AI dates are intentionally ignored; a different date must be
        // entered manually in the form.
        joined: todayStr(),
      }));
      setModalOpen(true);
    };
    const handleSave = () => voiceSubmitRef.current?.();

    window.addEventListener("AI_OPEN_TEACHER_MODAL", handleOpen);
    window.addEventListener("AI_UPDATE_TEACHER_FORM", handleOpen);
    window.addEventListener("AI_SAVE_TEACHER", handleSave);
    return () => {
      window.removeEventListener("AI_OPEN_TEACHER_MODAL", handleOpen);
      window.removeEventListener("AI_UPDATE_TEACHER_FORM", handleOpen);
      window.removeEventListener("AI_SAVE_TEACHER", handleSave);
    };
  }, [batches]);
  ...
  useEffect(() => {
    const update = (e) => {
      ...
      const resolveIds = (currentIds = []) => {
        const names = [];
        if (d.batch) names.push(String(d.batch));
        if (Array.isArray(d.batchNames)) names.push(...d.batchNames.map(String));
        const ids = batches.filter((b) => names.some((n) => String(b.name).trim().toLowerCase() === n.trim().toLowerCase())).map((b) => b._id);
      ...
      setEditForm((c) => ({
        ...
        ...(d.joiningDate !== undefined ? { joined: d.joiningDate } : {}),
        ...(d.accountEmail !== undefined ? { accountEmail: d.accountEmail } : {}),
        ...(d.accountPassword !== undefined ? { accountPassword: d.accountPassword } : {}),
        ...((d.batch !== undefined || Array.isArray(d.batchNames)) ? { batchIds: resolveIds(c.batchIds || []) } : {}),
      }));
    };
    ...
    window.addEventListener("AI_UPDATE_EDIT_FORM", update); window.addEventListener("AI_SAVE_EDIT", save); window.addEventListener("AI_CANCEL_EDIT", cancel);
  ...
  useEffect(() => {
    ...
    const execute = async (operation, item) => {
      if (operation === 'view') { setViewTargetId(item.id); setDetailTab('slips'); result(`Opening ${item.name}.`); return; }
      if (operation === 'edit') { startEdit(item); result(`Opening ${item.name} for editing.`); return; }
      if (operation === 'delete') { try { await teachersApi.remove(item.id); setTeachers((prev) => prev.filter((t) => t.id !== item.id)); result(`${item.name} was deleted successfully.`); } catch (err) { result(`I could not delete ${item.name}: ${err.message}`); } return; }
      if (operation === 'activate' || operation === 'deactivate') {
        ...
        try { await teachersApi.update(item.id, { active: next }); loadTeachers(); result(`${item.name} was ${next ? 'activated' : 'deactivated'} successfully.`); } catch (err) { result(`I could not change ${item.name}'s status: ${err.message}`); }
      }
    };
    const handler = (e) => {
      if (loading) { setTimeout(() => handler(e), 350); return; }
      const { operation, query: rawQuery } = e.detail || {}; const q = norm(rawQuery); if (!q) return;
      let matches = teachers.filter((t) => norm(t.name) === q || norm(t.specialization) === q);
      if (!matches.length) matches = teachers.filter((t) => norm(t.name).includes(q) || norm(t.specialization).includes(q));
      if (!matches.length) { result(`I could not find a teacher matching ${rawQuery}.`); return; }
      if (matches.length > 1) { ambiguous({ entity: 'teacher', operation, manual: true, message: `I can see multiple teachers matching ${rawQuery}. For safety, please manually visit the teacher record and choose View, Edit, ${operation === 'delete' ? 'Delete' : operation === 'activate' || operation === 'deactivate' ? 'Activate/Deactivate' : 'the requested action'}.` }); return; }
      execute(operation, matches[0]);
    };
    const executeSelected = (e) => { const { entity, operation, candidate } = e.detail || {}; if (entity === 'teacher' && candidate?.id) { const item = teachers.find((t) => t.id === candidate.id); if (item) execute(operation, item); } };
    window.addEventListener('AI_ENTITY_ACTION', handler); window.addEventListener('AI_ENTITY_ACTION_EXECUTE', executeSelected);
