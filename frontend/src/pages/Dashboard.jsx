...
export default function Dashboard() {
  ...
  // Voice Assistant can select any dashboard quick filter and can also put
  // exact dates directly into the native From/To inputs.  This listener is
  // intentionally local to Dashboard so other pages cannot accidentally
  // consume a dashboard date command.
  useEffect(() => {
    const handleVoiceDashboardFilter = (event) => {
      const detail = event.detail || {};
      const allowed = new Set(["today", "thisMonth", "lastMonth", "all", "custom"]);
      const next = String(detail.filter || "").trim();
      if (!allowed.has(next)) return;

      setFilter(next);

      if (next === "custom") {
        const dateField = String(detail.dateField || "").trim();
        setCustomRange((current) => {
          if (dateField === "start") {
            return {
              start: detail.startDate != null ? String(detail.startDate) : "",
              end: "",
            };
          }
          if (dateField === "end") {
            return {
              start: current.start,
              end: detail.endDate != null ? String(detail.endDate) : "",
            };
          }
          return {
            start: detail.startDate != null ? String(detail.startDate) : "",
        ...
        // A quick-filter voice command must not leave an old custom range
        // behind if the user later returns to Custom.
        setCustomRange((current) => ({
          start: current.start,
          end: current.end,
        }));
    ...
    window.addEventListener("AI_DASHBOARD_FILTER", handleVoiceDashboardFilter);
    return () => window.removeEventListener("AI_DASHBOARD_FILTER", handleVoiceDashboardFilter);
  }, []);
